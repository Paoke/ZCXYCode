    ...
    <body>
        <article class="page-container">
            <form class="form form-horizontal" id="form-article-add">
                <input type="hidden" id="vied" name="vied" value="<?php echo ($arrArticle["vied"]); ?>">
                <input type="hidden" id="indeximg_position" name="indeximg_position" value="<?php echo ($arrArticle["indeximg"]); ?>">
                <input type="hidden" id="thumbimg_position" name="thumbimg_position" value="<?php echo ($arrArticle["thumbimg"]); ?>">
                <input type="hidden" id="content" name="content">
                <div class="row cl">
                    <label class="form-label col-xs-4 col-sm-2"><span class="c-red">* </span>文章标题：</label>
                    <div class="formControls col-xs-8 col-sm-5">
                        <input type="text" class="input-text" placeholder="" id="title" name="title" value="<?php echo ($arrArticle["title"]); ?>">
                    </div>
                </div>
                ...
                <div class="row cl">
                    ...
                    <div class="formControls col-xs-8 col-sm-5">
                        <select class="select" size="1" id="class" name="class">
                            <option value="0">请选择</option>
                            <?php if($arrArticle["class"] == 1): ?><option value="1" selected="selected">学院动态</option>
                            <?php else: ?><option value="1">学院动态</option><?php endif; ?>

                            <?php if($arrArticle["class"] == 2): ?><option value="2" selected="selected">产业智库</option>
                                <?php else: ?><option value="2">产业智库</option><?php endif; ?>

                            <?php if($arrArticle["class"] == 3): ?><option value="3" selected="selected">时事热点</option>
                                <?php else: ?><option value="3">时事热点</option><?php endif; ?>
                        </select>
                    </div>
                </div>
                <div class="row cl">
                    ...
                    <div class="formControls col-xs-8 col-sm-5">
                        <input type="text" class="input-text" value="<?php echo ($arrArticle["from"]); ?>" placeholder="" id="from" name="from">
                    </div>
                </div>
                <div class="row cl">
                    <label class="form-label col-xs-4 col-sm-2"><span class="c-red">* </span>文章作者：</label>
                    <div class="formControls col-xs-8 col-sm-5">
                        <input type="text" class="input-text" value="<?php echo ($arrArticle["author"]); ?>" placeholder="" id="author" name="author">
                    </div>
                </div>
                ...
                <div class="row cl">
                    ...
                    <div class="formControls col-xs-8 col-sm-2">
                        <input type="text"
                               onfocus="WdatePicker()"
                               id="time" name="time" value="<?php echo ($arrArticle["time"]); ?>" class="input-text Wdate">
                    </div>
                </div>
                ...
                <div class="row cl">
                    ...
                    <div class="formControls col-xs-8 col-sm-9">
                        ...
                        <p><br><img src="/Upload/<?php echo ($arrArticle["indeximg"]); ?>" id="indeximg_see" style="width: 341px;height: 440px;"></p>
                    </div>
                </div>
                <div class="row cl">
                    <label class="form-label col-xs-4 col-sm-2"><span class="c-red">* </span>列表图：</label>
                    <div class="formControls col-xs-8 col-sm-9">
                        <input type="file" class="thumbimg" name="thumbimg" onchange="upload('thumbimg')"><span class="c-red" id="success">(图片规格：244*182)</span>
                        <p><br><img src="/Upload/<?php echo ($arrArticle["thumbimg"]); ?>" id="thumbimg_see" style="width: 244px;height: 182px;"></p>
                    </div>
                </div>
                <div class="row cl">
                    <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>摘要：</label>
                    <div class="formControls col-xs-8 col-sm-5">
                        <textarea name="describe" id="describe" cols="" rows="" class="textarea" placeholder="少于60个字"
                                  onKeyUp="textarealength(this, 100)"><?php echo ($arrArticle["describe"]); ?></textarea>
                    </div>
                </div>
                <div class="row cl">
                    <label class="form-label col-xs-4 col-sm-2"><span class="c-red">* </span>文章内容：</label>
                    <div class="formControls col-xs-8 col-sm-9">
                        <script id="editor" type="text/plain" style="width:748px;height:800px;">
                            <?php echo ($arrArticle["content"]); ?>
                        </script>
                    </div>
                </div>
                <div class="row cl">
        ...
        <script type="text/javascript">
            ...
            $('#btn_submit').on('click', function () {
                ...
                if(data.title==""||data.class=="0"||data.from==""||data.describe==""||data.author==""||data.time==""||data.indeximg_position==""||data.thumbimg_position==""||data.content==""){
                    layer.msg('请把资料填完整', {icon: 7, time: 2000});
                    return false;
                }
                var vied=$('#vied').val();
                $.post("/index.php/Admin/View/editArticle?vied="+vied, data, function (ret) {
                    if (ret== 'true') {
